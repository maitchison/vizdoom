...
# --------------------------------------------------------------------------------------------------
    ...
    for id_factor in [1000, 100, 10, 1, 0.1, 0, -1]:
        for novelty in [100, 10, 1, 0.1, 0, -0.1, -1]:
            ...
            # pick random parameters
            jobs.append(
                ("hgs novelty={} id_factor={}".format(novelty, id_factor), {
                'num_stacks':               4,
                'discount_factor':          0.95,
                'replay_memory_size':       20000,
                'target_update':            100,
    ...
    for i in range(args.repeats):
        ...
        jobs.append(
            ...
            'target_update':            np.random.choice([50, 100, 300, 1000, 3000, 10000, 30000]),
    ...
    for i in range(args.repeats):
        ...
        config_file, learning_steps, gates = {
            "search_A_wh":
                ("scenarios/my_way_home.cfg",       20000, [(25, -0.15)])
        }[args.trial]
